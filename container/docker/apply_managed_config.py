...
# ---------------------------------------------------------------------------
# Env-var overrides for world difficulty settings.
# These are written into WorldDescription.json on every startup so that the
# Helm values are always the source of truth.  Set to "" / unset to leave
# the field untouched (the game will use its own preset default).
# ---------------------------------------------------------------------------
_FLOAT_PARAMS: dict[str, str | None] = {
    "MobHealthMultiplier":            os.getenv("WINDROSE_MOB_HEALTH_MULTIPLIER"),
    "MobDamageMultiplier":            os.getenv("WINDROSE_MOB_DAMAGE_MULTIPLIER"),
    "ShipHealthMultiplier":           os.getenv("WINDROSE_SHIP_HEALTH_MULTIPLIER"),
    "ShipDamageMultiplier":           os.getenv("WINDROSE_SHIP_DAMAGE_MULTIPLIER"),
    "BoardingDifficultyMultiplier":   os.getenv("WINDROSE_BOARDING_DIFFICULTY_MULTIPLIER"),
    "Coop_StatsCorrectionModifier":   os.getenv("WINDROSE_COOP_STATS_CORRECTION_MODIFIER"),
    "Coop_ShipStatsCorrectionModifier": os.getenv("WINDROSE_COOP_SHIP_STATS_CORRECTION_MODIFIER"),
}
_BOOL_PARAMS: dict[str, str | None] = {
    "EasyExplore":      os.getenv("WINDROSE_EASY_EXPLORE"),
    "Coop.SharedQuests": os.getenv("WINDROSE_COOP_SHARED_QUESTS"),
}
_TAG_PARAMS: dict[str, str | None] = {
    "CombatDifficulty": os.getenv("WINDROSE_COMBAT_DIFFICULTY"),
}
...
# Optional server note — set via WINDROSE_SERVER_NOTE env var.
server_note = _non_empty_string(os.getenv("WINDROSE_SERVER_NOTE"))
if server_note:
    server_desc["Note"] = server_note
...
# ---------------------------------------------------------------------------
world_name_override = _non_empty_string(os.getenv("WINDROSE_WORLD_NAME"))
world_preset = _non_empty_string(os.getenv("WINDROSE_WORLD_PRESET")) or "Medium"
...
world_doc = _load_json(
    WORLD_PATH,
    {
        ...
        "WorldDescription": {
            "IslandId": world_id,
            "WorldName": world_name_override or server_desc.get("ServerName") or "Windrose Crew",
            "CreationTime": 0,
            "WorldPresetType": world_preset,
...
world_desc["WorldName"] = _non_empty_string(
    world_name_override or world_desc.get("WorldName") or server_desc.get("ServerName") or "Windrose Crew"
)
world_desc["WorldPresetType"] = world_preset
...
# Apply difficulty env vars — only set keys that are explicitly configured.
float_params = world_settings["FloatParameters"]
for param_key, raw in _FLOAT_PARAMS.items():
    if raw is not None and raw != "":
        try:
            float_params[param_key] = float(raw)
        except ValueError:
            pass

bool_params = world_settings["BoolParameters"]
for param_key, raw in _BOOL_PARAMS.items():
    if raw is not None and raw != "":
        bool_params[param_key] = raw.lower() in ("1", "true", "yes")

tag_params = world_settings["TagParameters"]
for param_key, raw in _TAG_PARAMS.items():
    if raw is not None and raw != "":
        tag_params[param_key] = raw
